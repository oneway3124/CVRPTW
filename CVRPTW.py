...
m.setObjective(quicksum(
    quicksum(x[(i, j)] * dist_matrix_clear[(i, j)] for j in range(len(coordinates))) for i in range(len(coordinates))),
               GRB.MINIMIZE)
m.update()
...
plt.ylabel('y-coordinate')
plt.title('WRSN')
plt.plot([XX1, XX2], [YY1, YY2], color='red')
plt.show()
